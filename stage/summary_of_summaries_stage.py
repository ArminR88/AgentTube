"""Stage runner for extracting unique claims across videos."""
...
def run_stage(
    ...
    """
    Run the summary-of-summaries stage.

    Arguments:
        summary_dir (str | Path): Directory containing _summary.json files.
        output_dir (str | Path): Directory to save claims.json.
    ...
    Returns:
        dict[str, Any]: Stage results with claims and stats.
    ...
    # Write claims
    claims_path = write_claims(claims_draft, output_dir, tokens_used, cost)

    # Build stats
    stats = build_claims_stats(len(summary_records), claims_draft)

    return {
        "claims": claims_draft.claims,
...
def main() -> None:
    ...
    Example:
        $ python stage/summary_of_summaries_stage.py --summary-dir output_agenttube/2026-08-04/transcript_summary --output-dir output_agenttube/2026-08-04/claims
    """
    import argparse
    import json
    import os

    parser = argparse.ArgumentParser(description="Run the summary-of-summaries stage")
    parser.add_argument(
        "--summary-dir",
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory to save claims.json",
    )
    parser.add_argument(
        "--api-key",
    ...
    print(f"Unique claims extracted: {result['claim_count']}")
